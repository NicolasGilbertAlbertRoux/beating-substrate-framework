...
\section*{Arc 10 — La ligne droite finale : murs nommés et simplification}

\begin{prop}[Catastrophe de $\Lambda$ levée par l'invariance d'échelle]
La divergence $\Lambda\sim\Lambda_{\mathrm{UV}}^4$ provient d'imposer un cutoff. Le substrat critique étant invariant d'échelle (sans échelle UV), l'énergie du vide ne dépend que de l'échelle IR : $\rho\sim 1/L^2$, indépendante du cutoff ($\rho L^2=$ const). Avec $L=$ horizon, l'ordre observé (valeur holographique). La catastrophe disparaît ; reste l'identification $L=$ horizon.
\end{prop}
\statut{invariance d'échelle (criticité S31/S55) ; énergie sombre holographique}{Recadrage réel ; magnitude non dérivée}{S64}

\begin{prop}[Racine R1 — algèbre géométrique + battement]
Une seule algèbre de Clifford de l'espace-temps porte les vecteurs (grade 1 $\to$ mouveurs $\to$ Dirac 3+1, opérateur reconstruit $H^2=p^2+m^2$ à $10^{-17}$) et les bivecteurs (grade 2 $\to$ rotations/courbure $\to$ spin-2 ; fermeture $[S_{12},S_{23}]=2S_{31}$, résidu nul). Le battement est le grade-0 à $c$ (S34). La dynamique de Dirac 3+1 se construit si les mouveurs se composent via le produit géométrique ; les ondes gravitationnelles à $c$ émergent si la courbure prend la raideur du battement (et non du cisaillement, sous-luminique S57), donnant $\gamma=1$ (S58).
\end{prop}
\statut{une racine, deux grades ; produit géométrique et couplage courbure--battement posés (naturels)}{Conditionnel ; unifie deux posés}{S71, S72, S73}

\begin{prop}[Racine R2 — criticité et verrouillage cyclique]
Le seuil de verrouillage des cycles est le point critique $K=1$ où le substrat se tient ; au-delà, les états $1/1,1/2,1/3,\dots$ sont des verrous robustes (largeurs de Farey), fournissant le domaine des structures internes. Le contre-battement rétroactif stabilise un cycle autour d'une amplitude fixe (cycle limite).
\end{prop}
\statut{seuil $=$ criticité et cycle limite (dérivés)}{Mécanisme établi ; ne sélectionne pas seul le 3}{S67, S70}

\begin{prop}[Intrants SM irréductibles, et simplification $4\to 2{+}2$]
Couleur Z$_3$ (P1) et générations (P2) sont deux « 3 » physiquement indépendants : ils ne fusionnent pas (refus de la numérologie ; Z$_3$ n'est pas dérivable des phases du battement, qui sont Z$_2$, S74). P2 est \emph{branché sur la résolution} : la hiérarchie de verrous cycliques est infinie (largeurs décroissantes $1/2>1/3>1/4>\dots$), mais seuls les verrous plus larges que le seuil de résolution (crénelage, socle II) sont observables — trois au-dessus du seuil, les suivants dans le latent (générations plus lourdes, indétectables ; prédiction cohérente avec l'absence de 4\textsuperscript{e} génération légère). Les quatre posés se réduisent à deux racines (R1, R2 — affinant le socle I/III et II) plus ces deux intrants SM.
\end{prop}
\statut{réduction honnête ; P2 réduit à « seuil entre 3\textsuperscript{e} et 4\textsuperscript{e} verrou » (calibration physique)}{Établi ; 2 racines $+$ 2 intrants}{S69, S73, S74}

\begin{prop}[Foyer latent et clôture méta-axiomatique]
Le substrat étant multirésolution, toute structure \emph{interne et invisible} habite le latent : la couleur y est donc automatiquement interne (commutant avec Lorentz), ce qui dissout la tension interne/espace-temps. Le latent fournit le \emph{foyer} mais ne force pas SU(3) (hiérarchie d'échelles, non triplet de directions). Au terme, aucune structure qualitative ne reste sans foyer ; le seul mur irréductible est méta-théorique — toute théorie finit sur des axiomes posés, l'émergence se perdant dans l'infini des résolutions. L'identifier \emph{boucle} la construction (réorganisation cohérente et transparente, non théorie du tout).
\end{prop}
\statut{rasoir d'Ockham (foyer déjà existant) ; couleur posée non dérivée}{Posé naturalisé ; clôture honnête}{S75, S74}

\section*{Arc 11 — Les magnitudes, cartographiées}

\begin{prop}[Protocole récursif « est-ce vraiment libre ? »]
Pour tout paramètre déclaré libre, tester s'il est en réalité \emph{sélectionné} par une dynamique plus profonde, puis recommencer, jusqu'à atteindre soit un invariant forcé, soit une frontière démontrée. Appliqué aux magnitudes : masses $\to$ distance-à-la-criticité $t$ $\to$ coefficient de fermeture. La distance $t$ est elle-même un \emph{attracteur} ($t^\star=\sqrt{y_t/\gamma}$, sélectionné depuis tout départ), donc non réglée à la main.
\end{prop}
\statut{récursion dynamique ; corrige trois fermetures prématurées}{Méthode ; $t$ auto-organisé}{S77--S82}

\begin{prop}[Le coefficient de fermeture est forcé par le flux KT]
L'« équation de flux autonome $K\to K^\star$ » requise existe : c'est le flux de Kosterlitz--Thouless où le substrat siège déjà (S31/S33). Il possède un point fixe \emph{universel} $K^\star=2/\pi$ (saut universel ; $K_R\to 2/\pi$ par le haut, vérifié) et des coefficients forcés. Le coefficient de fermeture/rétroaction est donc \textbf{forcé}, non un paramètre libre.
\end{prop}
\statut{flux KT (coefficients $4\pi^3$, saut $2/\pi$) ; identification fermeture$=$raideur critique}{Forcé (invariant KT)}{S86}

\begin{prop}[Unifié en ontologie, un unique datum off-critique en magnitude]
Masses, $G$, $\Lambda$ sont trois projections d'un même substrat (ontologie unifiée), et tout coefficient de la chaîne est forcé. Mais un $K^\star=2/\pi$ d'ordre 1 donne $t^\star\!\sim\!0{,}63$, d'ordre 1 : \emph{aucune} hiérarchie. La séparation observée ($\sim 10^{19}$ pour $G$, $\sim 10^{-31}$ pour $\Lambda$) vit dans l'unique \textbf{donnée off-critique} — le couplage relevant nu, que nul point fixe ne fixe. Le cadre est clos jusqu'à ce datum (le problème hiérarchie/constante cosmologique, ouvert pour tous) et le mur méta-axiomatique : non une théorie terminée, une théorie entièrement \emph{cartographiée}.
\end{prop}
\statut{ontologie unifiée (forcée) ; hiérarchie $=$ datum off-critique (ouvert pour tous)}{Cartographié ; un seul nombre libre nommé}{S80, S82, S86}
...
\begin{description}[leftmargin=0.6cm,itemsep=2pt]
  \item Structure interne : réduite à deux intrants \emph{nommés} et posés — couleur Z$_3$ (P1) et troncature-à-trois des générations (P2) ; le spectre des masses reste \textbf{ouvert} (intrant SM).
  \item Dynamique de Dirac 3+1 : opérateur reconstruit exactement (S71) ; \textbf{construit} modulo le produit géométrique de l'espace (racine R1).
  \item Ondes gravitationnelles à $c$ / Einstein complet : \textbf{atteignables} si la courbure prend la raideur du battement (R1, S72).
  \item Magnitudes (masses, $G$, $\Lambda$) : \textbf{cartographiées} (Arc 11) — unifiées en ontologie, tout coefficient de fermeture \textbf{forcé} (point fixe KT, $2/\pi$, S86) ; reste un \textbf{unique datum off-critique} (le couplage relevant nu $=$ problème hiérarchie/constante cosmologique, ouvert pour tous ; catastrophe $\Lambda$ levée, S64).
  \item $\hbar$ et le résultat stochastique de Born. \textbf{Adoptés} (constante mesurée / règle commune).
\end{description}
